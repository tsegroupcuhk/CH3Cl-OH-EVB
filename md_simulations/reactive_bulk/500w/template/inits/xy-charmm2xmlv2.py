#!/home/wentao/anaconda2/envs/py3/bin/python
...
parmfiles = sys.argv[1:]


def getFieldPairs(fields):
    pairs = []
    for i in range(int(len(fields)/2)):
    ...
    def __init__(self, fields):
        ...
        if self.polarizable:
            ...
            if self.alpha < 0*nanometers**2/kilojoules_per_mole:
                ...
                sign = -1
            self.drudeCharge = float("%.5f"%(sign*math.sqrt(self.alpha*2*(500*kilocalories_per_mole/angstrom**2))))
...
for inputfile in parmfiles:
    for line in open(inputfile):
        ...
        if line == 'read rtf card':
            ...
            elif fields[0] == "AVERAGE3":
                rom, rb, theta = fields[1:]
                residue.average3.append((float(rom), float(rb), float(theta)))
            elif fields[0] == "AVERAGE2":
                w1, w2 = fields[1:]
                residue.average2.append((float(w1), float(w2)))
            elif fields[0]== 'CUSTOMBONDC3':
                custombondc3=float(fields[1])
        elif section == 'para':
            if fields[0] in ('BONDS', 'ANGLES', 'DIHEDRALS', 'IMPROPER', 'CMAP','NONBONDED', 'NBFIX' ,'SCALEFACTOR','MYCHARGES','THOLE'):
                category = fields[0]
                residue = None
            elif category == 'BONDS':
                ...
                if pn == 0:
                    ...
                    key = (fields[0], fields[1], fields[2], fields[3])
                    if key not in periodicimpropers:
                        periodicimpropers[key] = []
                    periodicimpropers[key].append((float(fields[4])*kilocalories_per_mole, int(fields[5]), float(fields[6])*degrees))
            elif category == 'CMAP':
                if cmap is None:
                    cmap = Cmap(fields)
                    cmaps.append(cmap)
                ...
                LJparmL.append(fields)
                nb = Nonbonded(fields)
                nonbondeds[nb.atomClass] = nb
            elif category == 'NBFIX':
                nbfixes[(fields[0], fields[1])] = (-float(fields[2])*kilocalories_per_mole, float(fields[3])*angstroms)
            elif category == 'SCALEFACTOR':
                scaleL.append([fields[0],float(fields[1])**0.5]) 
            elif category == 'MYCHARGES':
                 match1=re.search("(\d+)(\w+)",fields[0])   
                 match2=re.search("(\d+)(\w+)",fields[1])
                 coeff1=int(match1.group(1))
                 name1=match1.group(2)
                 coeff2=int(match2.group(1))
                 name2=match2.group(2)
                 charge1=float(fields[2])
                 totalcharge=float(fields[3])
                 charge2=(totalcharge-coeff1*charge1)/(1.*coeff2)
                 mycharges[name1]=charge1
                 mycharges[name2]=charge2


#with open("selfLJ.json","w") as outf:
#     json.dump(LJparmL,outf)
...
for residue in residues:
    ...
    typeL[residue.name]=[]
    for atom in residue.atoms:
       typeL[residue.name].append(atom.type)

# Get durde_elements and parent_elements
drude_elements = []
parent_elements = []
for key in drudes:
    (drudeid, parentid) = key
    drude_ele = atomTypes[drudeid].name
    parent_ele = atomTypes[parentid].name
    drude_elements.append(drude_ele)
    parent_elements.append(parent_ele)
...
for type in atomTypes:	
    ...
    if mass == 0.0:
        if elemtype in drude_elements:
            mass = 0.4
        elementSpec = ''
        masslessTypes.add(type.type)
    else:
        if elemtype in parent_elements:
            mass = mass - 0.4
...
for residue in residues:
    ...
    for ave3 in residue.average3:
        rom, rb, theta = ave3
        theta = theta / 180.0 * math.pi
        ohmid = rb * math.cos(theta/2)
        wh = rom / 2. / ohmid
        wo = 1. - 2. * wh
        print('   <VirtualSite type="average3" index="1" atom1="0" atom2="2" atom3="3" weight1="%.8f" weight2="%.8f" weight3="%.8f" />'%(wo, wh, wh))
    for ave2 in residue.average2:
        w1, w2 = ave2
        print('   <VirtualSite type="average2" index="1" atom1="0" atom2="2" weight1="%.8f" weight2="%.8f"/>'%(w1, w2))
    print('  </Residue>')
print(' </Residues>')
print(' <HarmonicBondForce>')
for bond in bonds:
    #replace 'X' with '', which is wildcard in openmm
    newbond = [x.replace('X', '') for x in bond[:2]]
    print('  <Bond class1="%s" class2="%s" length="%.12f" k="%.12f"/>' % (newbond[0], newbond[1], bond[3].value_in_unit(nanometer), bond[2].value_in_unit(kilojoules_per_mole/nanometer**2)))
print(' </HarmonicBondForce>')
print(' <HarmonicAngleForce>')
for angle in angles:
    #replace 'X' with '', which is wildcard in openmm
    newangle = [x.replace('X', '') for x in angle[:3]]
    print('  <Angle class1="%s" class2="%s" class3="%s" angle="%.12g" k="%.12g"/>' % (newangle[0], newangle[1], newangle[2], angle[4].value_in_unit(radian), angle[3].value_in_unit(kilojoules_per_mole/radian**2)))
for angle in ubs:
    #replace 'X' with '', which is wildcard in openmm
    newangle = [x.replace('X', '') for x in angle[:3]]
    print('  <Angle class1="%s" class2="%s" class3="%s" angle="%.12g" k="%.12g"/>' % (newangle[0], newangle[1], newangle[2], angle[4].value_in_unit(radian), angle[3].value_in_unit(kilojoules_per_mole/radian**2)))
...
for dihedral in dihedrals:
    ...
    #replace 'X' with '', which is wildcard in openmm
    newdihedral = [x.replace('X', '') for x in dihedral[:4]]
    print('  <Proper class1="%s" class2="%s" class3="%s" class4="%s"%s/>' % (newdihedral[0], newdihedral[1], newdihedral[2], newdihedral[3], params))
#add periodic impropers
for pimproper in periodicimpropers:
    values = periodicimpropers[pimproper]
    params = ''
    for (i, (k, n, phase)) in enumerate(values):
        params += ' periodicity%d="%d" phase%d="%.12g" k%d="%g"' % (i+1, n, i+1, phase.value_in_unit(radians), i+1, k.value_in_unit(kilojoules_per_mole))
    #replace 'X' with '', which is wildcard in openmm
    newpimproper = [x.replace('X', '') for x in pimproper[:4]]
    #put center(3rd in old) at 1st
    print('  <Improper class1="%s" class2="%s" class3="%s" class4="%s"%s/>' % (newpimproper[2], newpimproper[0], newpimproper[1], newpimproper[3], params))
print(' </PeriodicTorsionForce>')
...
for improper in impropers:
    #replace 'X' with '', which is wildcard in openmm
    newimproper = [x.replace('X', '') for x in improper[:4]]
    print('  <Improper class1="%s" class2="%s" class3="%s" class4="%s" k="%g" theta0="%.12g"/>' % (newimproper[0], newimproper[1], newimproper[2], newimproper[3], improper[4].value_in_unit(kilojoules_per_mole/radian**2), improper[5].value_in_unit(radian)))
print(' </CustomTorsionForce>')


# add <CustomBondForce> and <CustomAngleForce> belong to swm4fw.xml
print(' <CustomBondForce energy="D*(a^2*(r-r0)^2-a^3*(r-r0)^3+%f*a^4*(r-r0)^4)">'%custombondc3)
print('  <PerBondParameter name="D"/>')
print('  <PerBondParameter name="a"/>')
print('  <PerBondParameter name="r0"/>')
print('  <Bond class1="ODW" class2="HDW" D="256.59117321723573" a="30.0" r0="0.09457"/>')
print(' </CustomBondForce>')
print(' <CustomAngleForce energy="ktheta^2*dtheta^2*(1-0.80214*dtheta+0.183837*dtheta^2-0.13166366*dtheta^3+0.23709*dtheta^4);dtheta=theta-theta0">')
print('  <PerAngleParameter name="ktheta"/>')
print('  <PerAngleParameter name="theta0"/>')
print('  <Angle class1="HDW" class2="ODW" class3="HDW" ktheta="13.72327078704519" theta0="1.8936822384138474"/>')
...
lj14scale=0.5
print(' <NonbondedForce coulomb14scale="0.8333333333" lj14scale="%f">'%lj14scale)
for type in atomTypes:
    factor=1.0
    for item in scaleL:        
        if type.type in typeL[item[0]]:
            factor=item[1]
    charge=type.charge
    #print(type.atomClass)
    #print(mycharges)
    if type.atomClass in mycharges:
           charge=mycharges[type.atomClass]    
    print('  <Atom type="%d" charge="%g" sigma="1.0" epsilon="1.0"/>' % (type.type, factor*charge))
...
for class1 in atomClasses:
    for class2 in atomClasses:
        print('%.10g, ' % getLennardJonesParams(class1, class2, False)[0].value_in_unit(kilojoules_per_mole), end='')
    print()
print("""]
sigma = [""")
for class1 in atomClasses:
    for class2 in atomClasses:
        print('%.10g, ' % (sigmaScale*getLennardJonesParams(class1, class2, False)[1].value_in_unit(nanometers)), end='')
    print()
print("""]
epsilon14 = [""")
for class1 in atomClasses:
    for class2 in atomClasses:
        ...
        if (class1, class2) in nbfixes:
            scale= lj14scale
        print('%.10g, ' % (scale*getLennardJonesParams(class1, class2, True)[0].value_in_unit(kilojoules_per_mole)), end='')
    print()
print("""]
sigma14 = [""")
for class1 in atomClasses:
    for class2 in atomClasses:
        print('%.10g, ' % (sigmaScale*getLennardJonesParams(class1, class2, True)[1].value_in_unit(nanometers)), end='')
    print()
print("""]
